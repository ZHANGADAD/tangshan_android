...
public class WangriShiFragment extends BaseFragment implements OnClickListener {
    ...
    private boolean isFuture, isShowTitle;

    public WangriShiFragment(boolean isFuture) {
        this.isFuture = isFuture;
        isShowTitle = true;
    }

    public WangriShiFragment(WangtiPagerFragment fatherParent,
                             String titleText, String cityNum, boolean isFuture) {
        // TODO Auto-generated constructor stub
        isShowTitle = false;
        this.isFuture = isFuture;
        cityInfo = new MCityInfo();
        cityInfo.setsName(titleText);
        cityInfo.setsNum(cityNum);
    }
    ...
    private void configTitle() {
        // TODO Auto-generated method stub
        if (!isShowTitle) {
            viewParent.findViewById(R.id.lllll).setVisibility(View.GONE);
            return;
        }
    ...
    private void initTopView() {
        TextView textView = (TextView) viewParent.findViewById(R.id.tvPreAfter);
        if (isFuture) {
            textView.setText("未来24小时");
        } else {
            textView.setText("过去24小时");
        }
        ...
        if (isFuture) {
            zhuantiView.setVisibility(View.GONE);
            viewParent.findViewById(R.id.btChemizhuanti).setVisibility(
                    View.GONE);
        } else {
            viewParent.findViewById(R.id.btChemizhuanti).setVisibility(
                    View.VISIBLE);
    ...
    protected void computeScroll(int x, int offSet, int range, int extent) {
        ...
        if (isFuture) {
            if (remenView.getVisibility() == View.VISIBLE) {
                ...
                tvTips.setText(cityInfos1.get(index).getsName());
            }
        } else {
            if (remenView.getVisibility() == View.VISIBLE) {
    ...
    private void changeTab(int category) {
        ...
        if (!isFuture) {
            if (category == 0) {
                ...
                if (cityInfos != null) {
                    tvTips.setText(cityInfos.get(23).getsNum());
                }
            } else if (category == 1) {
                ...
                zhuantiView.setVisibility(View.INVISIBLE);
                if (cityInfos != null) {
                    tvTips.setText(cityInfos.get(23).getsName());
                }
            } else {
                zhuantiView.setVisibility(View.VISIBLE);
                remenView.setVisibility(View.INVISIBLE);
                toutiaoView.setVisibility(View.INVISIBLE);
                if (cityInfos != null) {
                    tvTips.setText(cityInfos.get(23).getContent());
                }
            }
            ...
            if (category == 0) {
                ...
                remenView.setVisibility(View.INVISIBLE);
                if (cityInfos1 != null) {
                    tvTips.setText(cityInfos1.get(0).getsName());
                }
            }
